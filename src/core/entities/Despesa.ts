...
export class Despesa {
  ...
  adicionarTag(tag: Tag): void {
    if (this._tags.some(t => t.equals(tag))) {
      throw new Error('Tag já está associada a esta despesa');
    }
    this._tags.push(tag);
    this._atualizadaEm = new Date();
  }
  ...
  removerTag(tag: Tag): void {
    ...
    if (index === -1) {
      throw new Error('Tag não está associada a esta despesa');
    }
  ...
  private validarDescricao(descricao: string): void {
    ...
    if (descricao.trim().length > 200) {
      throw new Error('Descrição da despesa não pode ter mais de 200 caracteres');
    }
  }

  private validarTags(tags: Tag[]): void {
    if (tags.length > 10) {
      throw new Error('Uma despesa não pode ter mais de 10 tags');
    }
  }
  ...
  static criar(
    ...
    return new Despesa(id, descricao, valor, data, tags);
  }
}